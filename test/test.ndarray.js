...
tape( 'the function has an arity of 7', function test( t ) {
	t.strictEqual( mskmaxabs.length, 7, 'has expected arity' );
	t.end();
});
...
tape( 'the function calculates the maximum absolute value of a strided array according to a mask', function test( t ) {
	...
	mask = [ 0, 0, 0, 1, 0, 0, 0 ];
	v = mskmaxabs( x.length, x, 1, 0, mask, 1, 0 );
...
tape( 'the function calculates the maximum absolute value of a strided array according to a mask (accessor)', function test( t ) {
	...
	x = [ 1.0, -2.0, -4.0, NaN, 5.0, 0.0, 3.0 ];
	mask = [ 0, 0, 0, 1, 0, 0, 0 ];
	v = mskmaxabs( x.length, toAccessorArray( x ), 1, 0, toAccessorArray( mask ), 1, 0 );
	t.strictEqual( v, 5.0, 'returns expected value' );
...
tape( 'if provided an `N` parameter less than or equal to `0`, the function returns `NaN`', function test( t ) {
	...
	v = mskmaxabs( 0, x, 1, 0, mask, 1, 0 );
	t.strictEqual( isnan( v ), true, 'returns expected value' );

	v = mskmaxabs( -1, x, 1, 0, mask, 1, 0 );
	t.strictEqual( isnan( v ), true, 'returns expected value' );
...
tape( 'the function supports `stride` parameters (accessor)', function test( t ) {
	...
	v = mskmaxabs( 5, toAccessorArray( x ), 2, 0, toAccessorArray( mask ), 2, 0 );
...
tape( 'the function supports negative `stride` parameters (accessor)', function test( t ) {
	...
	v = mskmaxabs( 5, toAccessorArray( x ), -2, 8, toAccessorArray( mask ), -2, 8 );
...
tape( 'the function supports `offset` parameters (accessor)', function test( t ) {
	...
	v = mskmaxabs( 5, toAccessorArray( x ), 2, 1, toAccessorArray( mask ), 2, 1 );
	t.strictEqual( v, 4.0, 'returns expected value' );
